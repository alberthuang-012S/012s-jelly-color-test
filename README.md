...
The audit includes a 12,000-case production validation matrix (4 directions × 100 targets × 10 distances × 3 deterministic seeds) and 500-session virtual-observer simulations for thresholds 0.010, 0.020 and 0.040. See [`docs/measurement-engine-audit.md`](docs/measurement-engine-audit.md) for the recorded run.

## Measurement model

- Adaptive distance: nominal CIE 1976 `u′v′`; CIEDE2000 is retained for QA only.
- Test flow: 2 control trials → per-direction calibration → four interleaved 2-down/1-up tracks → eight repeated-level anchor trials → analysis.
...
### Engine revision: `uv8-glyph-clarity-tail`
...
- Each direction runs for at least 10 and at most 14 adaptive trials; 5 reversals indicate convergence. Hitting 14 trials stops a track with low convergence quality, not a claim of convergence. The former 35-trial session cap is removed. Four measurement axes and their anchors produce an estimated 54–78 total trials, depending on calibration and stopping points.
...
- Phase 2A adds a required environment confirmation, an intentional pause that hides the plate and restarts the current trial clock on resume, player-facing phase/progress labels, and a read-first result summary. The audit now covers the full 4 directions × 100 target numbers × 10 distances (including limits) × 3 deterministic seeds. Its open-response observer has no assumed 50% guessing floor, uses slope 4 on log-distance, and has 75% correct at the configured threshold. Three 500-session groups must each have ≥90% usable estimates, absolute median bias ≤25% of threshold, MAE ≤35%, and no more than the engine's 78-trial upper bound. These are engineering regression gates, not clinical acceptance criteria. Real observers, lapse/guess behavior, display calibration, and repeatability still require empirical validation. The prior audit reports apply only to earlier engine revisions and are archived in the commit history, not represented as current-engine evidence.
